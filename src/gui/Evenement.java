...
import entity.Event;
import houssem.services.ServiceEvent;
...
public class Evenement extends Form {    
   ...
   public Evenement(Form previeus) {
...
add(searchButton);
        ...
        for (Event event : events) {
            ...
            eventContainer.addComponent(dispoplaceEvent);
             ...
             reserverBtn.setUIID("LoginButton");
            Button qrBtn = new Button("Code QR");
            qrBtn.setUIID("LoginButton");
            
            // Ajouter des listeners d'événements aux boutons
reserverBtn.addActionListener(e -> {
    String url = DataSource.BASE_URL + "/reserver/94?idUser=28"; // Remplacez 94 par l'ID de l'événement approprié et 28 par l'ID de l'utilisateur approprié
...
trierButton.setUIID("LoginButton");
add(trierButton);
...
trierButton.addActionListener(e -> {
 ...
 qrBtn.addActionListener(e -> {
     ...
     String url = DataSource.BASE_URL + "qr-code/95";
